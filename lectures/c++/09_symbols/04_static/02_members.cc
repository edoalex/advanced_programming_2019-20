...
struct Foo {
  ...
  // static int si = 0; // error: must be const (if you want to initialize too)
  static const int sc = 9;  // only integral types are allowed (?)
  // you don't need to redefine sc outside the class,
  // because it's already initialized
  // static const double sd;  // error (r u sure a.s.? z actually fine)
  int i;
};
