...
// a macro is just one line, use \ to keep writing down

#define _AP_ERROR2(cond, exception_type)  if (!(cond))                         \
    // preprocessors substitutes definition on macro wherever it is
  ::internal::AssertHelper<exception_type>{} =                                 \
      // macro and not function so FILE and LINE are the right ones
      internal::MessageHandler{}                                               \
// first call AssertHelper struc, then MessageHandler struc
      << "\n\n"                                                                \
      << "------------------------------------------------------------------"  \
      << "\n"                                                                  \
